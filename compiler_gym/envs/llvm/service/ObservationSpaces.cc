  ...
  for (const auto& value : magic_enum::enum_values<LlvmObservationSpace>()) {
    ...
    switch (value) {
      ...
      case LlvmObservationSpace::INST_COUNT: {
        ...
        std::vector<int64_t> high(kInstCountFeatureDimensionality, std::numeric_limits<int64_t>::max());
        featureSizesHigh.mutable_values()->Add(high.begin(), high.end());

        observationSpace.set_deterministic(true);
        observationSpace.set_platform_dependent(false);

        observationSpace.mutable_default_observation()->mutable_int64_tensor()->mutable_shape()->Add(shape.begin(), shape.end());
        observationSpace.mutable_default_observation()->mutable_int64_tensor()->mutable_values()->Add(low.begin(), low.end());
        break;
      }
