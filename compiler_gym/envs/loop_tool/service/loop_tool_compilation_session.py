...
class LoopToolCompilationSession(CompilationSession):
    ...
    action_spaces = [
        ...
        ActionSpace(
            space=Space(
                ...
                name="split",
                named_discrete=NamedDiscreteSpace(
                    names=["toggle_mode", "up", "down", "toggle_thread", "split"],
                ),
            )
        ),
    ]
    ...
    observation_spaces = [
        ObservationSpace(
            space=Space(name="flops", double_value=DoubleRange()),
            deterministic=False,
        ...
        ObservationSpace(
            space=Space(
                name="action_state",
                int64_box=Int64Box(
                    low=Int64Tensor(shape=[1], values=[0]),
                    high=Int64Tensor(shape=[1], values=[2 ** 36]),
                ),
            ),
            deterministic=True,
            platform_dependent=False,
            default_observation=Event(
                int64_tensor=Int64Tensor(shape=[1], values=[0]),
            ),
        ),
    ]
    ...
    def apply_action(self, action: Event) -> Tuple[bool, Optional[ActionSpace], bool]:
        ...
        if choice_index < 0 or choice_index >= len(
            self.action_space.space.named_discrete.names
        ):
            raise ValueError("Out-of-range")

        logger.info("Applied action %d", choice_index)

        act = self.action_space.space.named_discrete.names[choice_index]
    ...
    def get_observation(self, observation_space: ObservationSpace) -> Event:
        if observation_space.space.name == "action_state":
            # cursor, (size, tail)
            o = self.order[self.cursor]
            return Event(
                int64_tensor=Int64Tensor(shape=[3], values=[self.cursor, o[0], o[1]])
            )
        elif observation_space.space.name == "flops":
            return Event(double_value=self.flops())
        elif observation_space.space.name == "loop_tree":
            loop_tree, parallel = self.lower()
            return Event(
