...
def convert_standard_tensor_message_to_numpy(
    ...
    res = np.array(tensor.values, dtype=type_to_dtype_map[type(tensor)])
    res = res.reshape(tensor.shape)
    return res


def convert_numpy_to_boolean_tensor_message(tensor: np.ndarray):
    return BooleanTensor(values=tensor.flatten().tolist(), shape=tensor.shape)
...
def convert_numpy_to_byte_tensor_message(tensor: np.ndarray):
    return ByteTensor(values=tensor.tobytes(), shape=tensor.shape)


def convert_numpy_to_int64_tensor_message(tensor: np.ndarray):
    return Int64Tensor(values=tensor.flatten(), shape=tensor.shape)


def convert_numpy_to_float_tensor_message(tensor: np.ndarray):
    return FloatTensor(values=tensor.flatten(), shape=tensor.shape)


def convert_numpy_to_double_tensor_message(tensor: np.ndarray):
    return DoubleTensor(values=tensor.flatten(), shape=tensor.shape)


def convert_numpy_to_string_tensor_message(tensor: np.ndarray):
    return StringTensor(values=tensor.flatten(), shape=tensor.shape)
...
class ListEventMessageConverter:
    ...
    def __call__(self, list_event: ListEvent) -> List[Any]:
        return [self.event_message_converter(event) for event in list_event.events]
...
class ToListEventMessageConverter:
    ...
    def __call__(self, event_list: List) -> ListEvent:
        return ListEvent(
            events=[self.to_event_converter(event) for event in event_list]
        )
...
class DictEventMessageConverter:
    ...
    def __call__(self, dict_event: DictEvent) -> DictType[str, Any]:
        return {
            ...
            for key, event in dict_event.events.items()
        }
...
class ToDictEventMessageConverter:
    ...
    def __call__(self, d: DictType) -> DictEvent:
        return DictEvent(
            events={key: self.to_event_converter(val) for key, val in d.items()}
        )
...
class ListSpaceMessageConverter:
    ...
    def __call__(self, list_space: ListSpace) -> Tuple:
        return Tuple(
            spaces=[self.space_message_converter(space) for space in list_space.spaces],
            name=None,
        )
...
class ToListSpaceMessageConverter:
    ...
    def __call__(self, spaces: Tuple) -> ListSpace:
        return ListSpace(
            spaces=[self.to_space_converter(space) for space in spaces.spaces]
        )
...
class ToDictSpaceMessageConverter:
    ...
    def __call__(self, d: Dict) -> DictSpace:
        return DictSpace(
            spaces={key: self.to_space_converter(val) for key, val in d.spaces.items()}
        )
